...
function update_grow(grow, grow_id) {
  ...
  $(grow_id)
    .text(text)
    .removeClass('green__text')
    .removeClass('red__text')
...
function update_simple_indicator(stats, indicator_id, dom_node_id, format_number_func = format_number) {
  ...
  $(dom_node_id).text(format_number_func(ind_val[1]));

  $(dom_node_id + '_year').text(stats.years[ind_val[0]]);
...
function update_area_chart(stats, sources, dom_node_id, font_size = 16, format_number_func = format_number) {
  ...
  Array.from(datasets).forEach(function (item, i) {
    ...
    series.push({
      type: 'areaspline',
      pointStart: Date.UTC(new Date().getFullYear() - 9, 0, 1),
      pointInterval: 24 * 60 * 60 * 60 * 1000,
  ...
  if (!(chartContainer.attr('data-reverse') == 'false')) series.reverse();
  ...
  var chart = Highcharts.chart(dom_node_id, {
    ...
    xAxis: {
      type: 'datetime',
      tickInterval: 24 * 60 * 60 * 60 * 1000 * 5,
